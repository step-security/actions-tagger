...
describe('Functions', () => {
  describe.each([
    ...
    test(`when INPUT_PREFER_BRANCH_RELEASES=${preferbranchReleases} returns ${expected}`, async () => {
      ...
      await import('@actionstagger/functions/private').then(
  ...
  describe.each([
    ...
    '#findLatestRef(github)',
    ({ pushedRef, existing, repoLatest, expected }) => {
      beforeEach(async () => {
        ...
        await import('@actionstagger/functions').then(functions => {
          jest
            .spyOn(functions.default, 'getPublishRefVersion')
            .mockReturnValue(semverTag);
        });

        await import('@actionstagger/functions/private').then(functions => {
          jest
            .spyOn(functions.default, 'listAllPublicRefs')
      ...
      test(`when new release or push of ${pushedRef}, and ref with name ${
        ...
        await findLatestMajorRef(getOctokit('TEST_TOKEN')).then(
          ({ isLatest, majorLatest }) => {
  ...
  describe.each([
    ...
    beforeEach(async () => {
      ...
      await import('@actionstagger/functions/public').then(
        ({ default: functions }) => {
          jest
            .spyOn(functions, 'getPublishRefVersion')
            .mockReturnValue(semverParse(ref));
        }
      );
    });
  ...
  describe.each([
    ...
    ({ refToCreate, isLatest, expectedRef }) => {
      beforeEach(async () => {
        ...
        await import('@actionstagger/functions/private').then(functions =>
          jest
            .spyOn(functions.default, 'getPreferredRef')
            .mockReturnValue('tags')
        );
      });
